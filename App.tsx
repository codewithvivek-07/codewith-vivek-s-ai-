...
export default function App() {
  ...
  const startNewChat = (nameOverride?: string) => {
    ...
    const welcomeMsg: Message = { id: 'boot', role: Role.MODEL, text: `Hello. I am ${nameOverride || aiName}. How can I assist you today?`, timestamp: Date.now() };
  ...
  const handleNameSave = () => {
    ...
    setIsNaming(false);
    if (sessions.length === 0) startNewChat(tempNameInput);
  };
  ...
  const themeColors = useMemo(() => {
     ...
     return {
        ...
        border: 'border-[rgb(var(--theme-primary-rgb),0.3)]',
        
        userBubble: isAdmin 
          ? 'bg-[rgb(var(--theme-admin-rgb),0.1)] border border-[rgb(var(--theme-admin-rgb),0.3)] text-[var(--color-text-base)] rounded-2xl rounded-tr-xl' 
          : 'bg-[var(--color-input-bg)] border border-[var(--color-border-glass)] text-[var(--color-text-base)] rounded-2xl rounded-tr-xl',
        
        aiBubble: 'bg-[var(--color-bg-glass)] border border-[var(--color-border-glass)] text-[var(--color-text-base)] rounded-2xl rounded-tl-xl',
        
        button: isAdmin
          ? 'bg-[rgb(var(--theme-admin-rgb))] text-white hover:opacity-90 shadow-ios'
          : 'bg-[rgb(var(--theme-primary-rgb))] text-white hover:opacity-90 shadow-ios',
        
        accent: isAdmin ? 'text-[rgb(var(--theme-admin-rgb))]' : 'text-[rgb(var(--theme-primary-rgb))]',
        
        adminGlow: isAdmin ? 'admin-glow' : '',
        toggleActive: isAdmin ? 'bg-[rgb(var(--theme-admin-rgb))]' : 'bg-[rgb(var(--theme-primary-rgb))]',
     };
  ...
  return (
    <div className={`flex h-screen font-sans overflow-hidden bg-[var(--bg-body)] text-[var(--color-text-base)] transition-colors duration-300`}>
      ...
      <div className={`flex-1 flex flex-col h-full relative z-10 min-w-0`}>
        {/* Header - iOS Glass */}
        <header className="flex-none h-16 px-6 flex items-center justify-between z-20 glass-panel border-b-[var(--color-border-glass)] m-4 rounded-2xl">
          <div className="flex items-center gap-4">
             <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`p-2 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)]`}>
          ...
          <div className="flex items-center gap-2">
            {/* Dark/Light Toggle */}
            <button onClick={() => setSettings({...settings, theme: settings.theme === 'dark' ? 'light' : 'dark'})} className="p-2 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)]">
                {settings.theme === 'dark' ? (
            ...
            <button onClick={() => setIsPlaygroundOpen(true)} className="p-2 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)]">
            ...
            <button onClick={toggleAdmin} className={`p-2 rounded-full transition-colors ${isAdmin ? `text-[rgb(var(--theme-admin-rgb))] hover:bg-[rgb(var(--theme-admin-rgb),0.1)] ${themeColors.adminGlow}` : 'text-[var(--color-text-muted)] hover:bg-[var(--color-input-bg)]'}`}>
               <span className="text-xl leading-none">😈</span>
            </button>
            <button onClick={() => setShowSettings(true)} className="p-2 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)]">
            ...
            <button onClick={() => setIsModalOpen(true)} className="p-2 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)]">
        ...
        {/* Chat Area */}
        <main ref={chatContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 scroll-smooth">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col max-w-4xl mx-auto ${msg.role === Role.USER ? 'items-end' : 'items-start'}`}>
              <div className="flex items-end gap-3 max-w-full group relative">
                {msg.role === Role.MODEL && (
                   <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-[var(--color-input-bg)] shadow-sm flex-shrink-0`}>
                ...
                <div className={`relative px-5 py-4 text-[15px] leading-relaxed shadow-sm
                  ...
                  {msg.appContent && (
                    <div className="my-3 p-4 rounded-2xl glass-panel border border-[var(--color-border-glass)]">
                        ...
                        <div className="flex gap-2 mb-4">
                            ...
                            <button onClick={() => handleDownloadZip(msg.appContent!)} className="px-3 py-1.5 rounded-lg border border-[var(--color-border-glass)] hover:bg-[var(--color-input-bg)] text-xs font-bold transition-colors">Download ZIP</button>
                        </div>
        ...
        <footer className="flex-none p-4 sm:p-6 z-20">
           <div className="max-w-4xl mx-auto relative glass-panel rounded-3xl p-3 flex items-center gap-2 shadow-2xl">
              {showCommandMenu && (
                 <div ref={commandMenuRef} className="absolute bottom-full left-0 mb-4 w-56 glass-panel rounded-2xl overflow-hidden animate-fade-in flex flex-col p-2">
                    {['/make', '/image', '/build', '/playground'].map(cmd => (
                      <button key={cmd} onClick={() => { cmd === '/playground' ? setIsPlaygroundOpen(true) : setInput(cmd + ' '); setShowCommandMenu(false); }} className="text-left px-4 py-2 hover:bg-[var(--color-input-bg)] rounded-xl text-sm font-medium transition-colors">{cmd === '/playground' ? 'Open Playground' : cmd}</button>
                    ))}
                 </div>
              )}
              {showPromptMenu && (
                 <div ref={promptMenuRef} className="absolute bottom-full right-0 mb-4 w-72 glass-panel rounded-2xl overflow-hidden animate-fade-in p-2">
                    <div className="flex justify-between px-3 py-2 text-xs font-bold opacity-50 uppercase"><span>Suggestions</span><button onClick={shufflePrompts}>↻</button></div>
                    {visiblePrompts.map((p, i) => (
                      <button key={i} onClick={() => { setInput(p.prompt); setShowPromptMenu(false); }} className="w-full text-left px-3 py-2 hover:bg-[var(--color-input-bg)] rounded-xl text-sm transition-colors truncate">{p.title}</button>
                    ))}
                 </div>
              )}

              <button onClick={() => setShowCommandMenu(!showCommandMenu)} className={`p-3 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)] ${isTyping ? 'hidden' : ''}`}><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg></button>
              
              <div className={`relative ${isTyping ? 'hidden' : ''}`}>
                 <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                 <button onClick={() => fileInputRef.current?.click()} className="p-3 rounded-full hover:bg-[var(--color-input-bg)] transition-colors text-[var(--color-text-muted)]"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg></button>
              </div>

              <input 
                type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && handleSend()}
                placeholder="Message AI..." className="flex-1 bg-transparent border-none focus:ring-0 text-[16px] placeholder-[var(--color-text-muted)] px-2"
              />

              <button onClick={toggleRecording} className={`p-3 rounded-full transition-colors ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'text-[var(--color-text-muted)] hover:bg-[var(--color-input-bg)]'} ${isTyping ? 'hidden' : ''}`}><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg></button>

              <button 
                onClick={() => handleSend()} disabled={!input.trim() && !attachment && !fileContext}
                className={`p-3 rounded-full transition-all shadow-lg ${(!input.trim() && !attachment && !fileContext) ? 'bg-gray-500/20 text-gray-500' : themeColors.button}`}
              >
                {isLoading ? <div className="h-5 w-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg>}
              </button>
           </div>
           <div className="text-center mt-2">
             <button onClick={() => setShowPromptMenu(!showPromptMenu)} className="text-xs font-medium opacity-40 hover:opacity-100 transition-opacity">Suggestions</button>
           </div>
        </footer>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4 animate-fade-in">
            <div className="glass-panel rounded-3xl p-8 max-w-sm w-full text-center">
               ...
               <form onSubmit={handleAdminSubmit} className="space-y-4">
                  <input type="password" value={adminInputCode} onChange={(e) => setAdminInputCode(e.target.value)} className="w-full bg-[var(--color-input-bg)] border-none rounded-xl px-4 py-3 text-center text-2xl tracking-[0.5em] focus:ring-2 focus:ring-[rgb(var(--theme-admin-rgb))]" autoFocus placeholder="••••••" />
                  <div className="flex gap-3">
                     <button type="button" onClick={() => setShowAdminInput(false)} className="flex-1 py-3 rounded-xl bg-[var(--color-input-bg)] font-bold text-sm">Cancel</button>
                     <button type="submit" className="flex-1 py-3 rounded-xl bg-[rgb(var(--theme-admin-rgb))] text-white font-bold text-sm shadow-lg">Unlock</button>
                  </div>
               </form>
            </div>
         </div>
      ...
      {isNaming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4 animate-fade-in">
          <div className="glass-panel rounded-3xl p-6 max-w-sm w-full shadow-2xl">
            <h3 className="text-lg font-bold mb-4 text-center">Identity Configuration</h3>
            <input type="text" value={tempNameInput} onChange={e => setTempNameInput(e.target.value)} className="w-full bg-[var(--color-input-bg)] border-none rounded-xl px-4 py-3 mb-4 text-center focus:ring-2 focus:ring-[rgb(var(--theme-primary-rgb))]" placeholder="Name your AI..." />
            <button onClick={handleNameSave} className={`w-full py-3 rounded-xl font-bold text-sm text-white shadow-lg ${themeColors.button}`}>Save Identity</button>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4 animate-fade-in">
           <div className="glass-panel rounded-3xl max-w-md w-full overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
              <div className="p-6 border-b border-[var(--color-border-glass)] flex justify-between items-center">
                <h3 className="text-xl font-bold">Settings</h3>
                <button onClick={() => setShowSettings(false)} className="p-2 bg-[var(--color-input-bg)] rounded-full text-xs font-bold">Done</button>
              </div>
              <div className="p-6 space-y-6 overflow-y-auto">
                 <div>
                    <label className="block text-xs font-bold opacity-60 uppercase mb-2">Custom Persona</label>
                    <textarea value={settings.customPersona} onChange={e => setSettings({...settings, customPersona: e.target.value})} className="w-full bg-[var(--color-input-bg)] border-none rounded-xl p-3 text-sm resize-none h-24 focus:ring-2 focus:ring-[rgb(var(--theme-primary-rgb))]" placeholder="How should I behave?" />
                 </div>
                 <div>
                    <label className="block text-xs font-bold opacity-60 uppercase mb-2">Color Palette</label>
                    <div className="grid grid-cols-2 gap-2">
                      {Object.keys(THEME_PALETTES).map(p => (
                        <button key={p} onClick={() => setSettings({...settings, currentPalette: p})} className={`p-3 rounded-xl text-xs font-bold transition-all ${settings.currentPalette === p ? themeColors.toggleActive + ' text-white shadow-lg' : 'bg-[var(--color-input-bg)] opacity-70'}`}>{THEME_PALETTES[p as keyof typeof THEME_PALETTES]}</button>
                      ))}
                    </div>
                 </div>
                 <div className="space-y-2">
                    {[
                      ...
                      <div key={item.l} className="flex items-center justify-between p-3 bg-[var(--color-input-bg)] rounded-xl">
                         ...
                         <button onClick={item.fn} className={`w-11 h-6 rounded-full p-0.5 transition-colors ${item.v ? themeColors.toggleActive : 'bg-gray-300 dark:bg-gray-700'}`}>
